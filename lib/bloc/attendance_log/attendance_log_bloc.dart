  ...
  AttendanceLogBloc() : super(AttendanceLogInitial()) {
    on<GetAttendanceLog>((event, emit) async {
      ...
      try {
        ...
        final List<Attendance> attendanceList =
            await AttendanceRepository().getHistoryAttendance(
          token: token,
          month: event.month,
          year: event.year,
        );
    ...
    on<ScrollFetch>((event, emit) async {
      ...
      try {
        ...
        final List<Attendance> newAttendanceList = await AttendanceRepository()
            .getHistoryAttendance(token: token, page: event.page.toString());
